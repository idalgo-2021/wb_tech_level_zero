...
3. При чтении данных из Kafka, невалидные сообщения и сообщения, заказы из которых уже имеются в системе - перекладывается в специализированный Kafka-топик DLQ(Dead Letter Queue). Т.о. сообщения не теряются(даже с дублями и невалидные, но основной топик не содержит "мусора"). 
    * Предполагается, что топик DLQ обрабатывается в отдельном порядке(в моем проекте эта обработка не реализована). 
    * Для невалидных и дублирующих сообщений, не предусмотрено дополнительных попыток записи в БД сервиса, они сразу же передаются в DLQ. Для всех остальных сообщений предусмотрено несколько попыток записи в систему(через увеличивающийся интервал времени).
...
В приложении используется `.env`-файл для загрузки переменных окружения, которые в свою очередь используются приложением и в подготовке Docker-контейнеров.  
