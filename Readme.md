...
- [Важные Замечания к выполнению задания](#Remarks)
...
2. Входящие сообщения(заказы) ищутся в системе по полю 'order_uid'. Если заказ уже имеется в системе, то сообщение не обрабатывается. Такое сообщение перекладывается в топик DLQ.

3. При чтении данных из Kafka, невалидные сообщения и сообщения, заказы из которых уже имеются в системе - перекладывается в специализированный Kafka-топик DLQ(Dead Letter Queue). Т.о. сообщения не теряются(даже с дублями и невалидные, но основной топик не содержит "мусора"). Это самая простая схема использования DLQ, но при необходимости её можно быстро изменить и адаптировать под требования.
    * Предполагается, что топик DLQ обрабатывается в отдельном порядке(в моем проекте эта обработка не реализована). 
    * Для невалидных и дублирующих сообщений, не предусмотрено дополнительных попыток записи в БД сервиса, они сразу же передаются в DLQ. Для всех остальных сообщений предусмотрено несколько попыток записи в систему(через увеличивающийся интервал времени). Данную механику тоже возможно улучшить, например расширить перечень ошибок которые требуют\не требуют повторных попыток. Но, механика в любом случае должна быть адекватна требованиям(а их нет).
...
1. В приложении используется `.env`-файл для загрузки переменных окружения, которые в свою очередь используются приложением и в подготовке Docker-контейнеров.  
...
Параметры  .env-файла:
      ...
      ORDER_CACHE_TTL_MINUTES=10

      KAFKA_BROKER=localhost:9094
      KAFKA_TOPIC=orders
      KAFKA_GROUP_ID=order-consumer
      KAFKA_CONSUMER_COUNT=2
...
│       └── params.go         - вспомогательные структуры
...
├── pgdata  [error opening dir]     - файлы БД(в контейнере Docker не храним)
├── pkg
│   ├── db
│   │   └── postgres.go       - конструктор подключения к PostgreSQL 
│   ├── logger
│   │   └── logger.go         - обертка над zap, логгер сервиса   
│   └── redisclient
│       └── redisclient.go    - конструктор клиента Redis
└── Readme.md
```
...
```
   ...
   # После генерации сообщений(заказов) в кафку, по их UID у вас так же должен работать и фронтенд `frontend/index.html`
